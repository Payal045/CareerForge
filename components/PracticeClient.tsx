...
import useRoadmaps from "@/app/api/hooks/useRoadmaps"; // client-safe hook (created earlier)
...
export default function PracticeClient({ defaultNodeId }: { defaultNodeId?: string }) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const paramPracticeKey = searchParams?.get?.("practiceKey") ?? null;
  const paramNodeId = searchParams?.get?.("nodeId") ?? null;
  const paramType = searchParams?.get?.("type") ?? null;
  const paramRoadmapId = searchParams?.get?.("roadmapId") ?? null;
  ...
  const DEFAULT_LANGUAGE_ID = 71; // Python3

  const { updateProgress } = useRoadmaps();

  const nodeId = paramNodeId ?? defaultNodeId ?? null;
  const type = paramType ?? null;
  const roadmapId = paramRoadmapId ?? null;
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        // session storage preseed
        if (paramPracticeKey) {
          const raw = typeof window !== "undefined" ? sessionStorage.getItem(paramPracticeKey) : null;
          if (raw) {
            try {
        ...
        if (nodeId && type) {
          ...
          if (r.ok) {
            ...
            console.error("Generator returned", r.status);
          }
        }
  ...
  }, [paramPracticeKey, paramNodeId, paramType, paramRoadmapId]);
