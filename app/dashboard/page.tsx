...
import { ArrowRight, Target, Zap, Trophy, LogOut, Search, FileText, Home, Trash2 } from "lucide-react";
import { useRouter, useSearchParams } from "next/navigation";
import RoadmapPage from "../roadmap/page";
import NotesPage from "../notes/page";
import useRoadmaps from "../api/hooks/useRoadmaps";
import PracticeSection from "@/components/PracticeSection";
import useStreak from "../api/hooks/useStreak";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // If only selected param present, set selection but keep previous section (do not auto-navigate)
    if (selectedParam) {
      setSelectedRoadmap(selectedParam);
    }
  ...
  // When a session becomes active, call touch() once with the user's local date (YYYY-MM-DD).
  // This updates streak by calendar day (dedupe handled inside useStreak).
  useEffect(() => {
    if (!session?.user) return;
    try {
      // Use en-CA locale to reliably produce YYYY-MM-DD in browsers
      const localDate = new Date().toLocaleDateString("en-CA");
      touch?.(localDate).catch((e) => {
        // non-fatal
        console.warn("streak touch failed:", e);
      });
    } catch (e) {
  ...
  // Updated handler: set selected roadmap id and open PracticeSection
  const handleContinue = (card: RoadmapCard) => {
    setSelectedRoadmap(card.id);
    setActiveSection("practice");

    // reflect selection in URL so it's shareable/bookmarkable
    const q = new URLSearchParams(window.location.search);
    q.set("selected", String(card.id));
    // don't force openPractice in this action — keep navigation in-app
  ...
  const handleDelete = async (card: RoadmapCard) => {
    ...
    try {
      ...
      // if we deleted the currently selected roadmap, clear practice view
  ...
  <div>
    ...
    <div className="text-lg font-semibold">{streakLoading ? (
